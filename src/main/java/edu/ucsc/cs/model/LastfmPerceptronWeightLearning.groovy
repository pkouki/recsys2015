...
average_rating[0]= 0.565892	//these are the outputs of the awk script for the 90 percent of the training file
average_rating[1]= 0.566533
average_rating[2]= 0.565971
average_rating[3]= 0.565724
average_rating[4]= 0.565386
...
//ITEMS
//pearson items
m.add rule :  ((rated(U,I1) & rated(U,I2) & rating(U,I1) & sim_pearson_items(I1,I2)) >> rating(U,I2)), weight: 3, squared:false;

//cosine items
m.add rule :  ((rated(U,I1) & rated(U,I2) & rating(U,I1) & sim_cosine_items(I1,I2)) >> rating(U,I2)), weight: 3, squared:false;

//adj cosine items
m.add rule :  ((rated(U,I1) & rated(U,I2) & rating(U,I1) & sim_adjcos_items(I1,I2)) >> rating(U,I2)), weight: 3, squared:false;

//USERS
//pearson users
m.add rule :  ((rated(U1,I) & rated(U2,I) & rating(U1,I) & sim_pearson_users(U1,U2)) >> rating(U2,I)), weight: 3, squared:false;

//cosine users
m.add rule :  ((rated(U1,I) & rated(U2,I) & rating(U1,I) & sim_cosine_users(U1,U2)) >> rating(U2,I)), weight: 3, squared:false;
...
m.add rule :  ((user(U1) & user(U2) & item(I) & rating(U1,I) & rated(U1,I) &rated(U2,I) & sim_mf_cosine_users(U1,U2))
			>> rating(U2,I)), weight : 3, squared:false;
...
m.add rule :  ((user(U) & item(I1) & item(I2) & rating(U,I1) & rated(U,I1) & rated(U,I2) & sim_mf_cosine_items(I1,I2))
			>> rating(U,I2)), weight: 3, squared:false;
...
//PREDICTIONS
//SGD Predictions
m.add rule: ((sgd_rating(U,I)) >> rating(U,I)), weight:3, squared:sq;
m.add rule: ((rating(U,I)) >> sgd_rating(U,I)), weight:3, squared:sq;

//item based pearson Predictions
m.add rule: ((item_pearson_rating(U,I)) >> rating(U,I)), weight:3, squared:sq;
m.add rule: ((rating(U,I)) >> item_pearson_rating(U,I)), weight:3, squared:sq;

//BPMF predictions
m.add rule: ((bpmf_rating(U,I)) >> rating(U,I)), weight:3, squared:sq;
m.add rule: ((rating(U,I)) >> bpmf_rating(U,I)), weight:3, squared:sq;

//AVERAGE PRIORS
m.add rule : ((user(U) & item(I) & rated(U,I) & avg_user_rating(U)) >> rating(U,I)), weight: 3, squared:sq;
m.add rule : ((user(U) & item(I) & rated(U,I) & rating(U,I)) >> avg_user_rating(U)), weight: 3, squared:sq;

//items tend to get ratings towards their average
m.add rule : ((user(U) & item(I) & rated(U,I) & avg_item_rating(I)) >> rating(U,I)), weight: 3, squared:sq;
m.add rule : ((user(U) & item(I) & rated(U,I) & rating(U,I)) >> avg_item_rating(I)), weight: 3, squared:sq;

//SOCIAL RULES
//friendships
m.add rule: ((rated(U1,I) & rated(U2,I) & users_are_friends(U1,U2) & rating(U1,I)) >> rating(U2,I)), weight: 3, squared:false;

//CONTENT RULES

m.add rule :  ((rated(U,I1) & rated(U,I2) & rating(U,I1) & sim_content_items_jaccard(I1,I2)) >> rating(U,I2)), weight: 3, squared:false;


// Two-sided overall prior
UniqueID constant = data.getUniqueID(0)
m.add rule: ( user(U) & item(I) & rated(U,I) & rating_prior(constant) ) >> rating(U,I), weight: 3, squared:sq;
m.add rule: ( user(I) & item(I) & rated(U,I) & rating(U,I) ) >> rating_prior(constant), weight: 3, squared:sq;
...
int ext = 60;
def dir = 'data'+java.io.File.separator+'lastfm'+java.io.File.separator + 'binomialFolds'+ java.io.File.separator;
def w_dir = "weight_learning" + java.io.File.separator;
...
String bpmf_sim = "bpmf-similarities" + java.io.File.separator;
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + w_dir + "f" + fold + ".lastfm.train.clean");
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + w_dir + "sgd.predictions.100.factors.fold." + fold);
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + w_dir + "item.based.pearson.top.1.predictions.fold." + fold);
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + w_dir + "f" + fold + ".lastfm.test.clean");
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + "f" + fold + ".lastfm.train.clean");
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + graphlab + "sgd.predictions.100.factors.fold." + fold);
	...
	InserterUtils.loadDelimitedDataTruth(insert, dir + "sim.items.content.jaccard.top.50.boolean");
	...
	def labels = new File(dir + "f" + fold + ".lastfm.test.clean")
